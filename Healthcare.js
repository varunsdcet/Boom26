...
import { Platform,StyleSheet, Text,FlatList,StatusBar, View ,Image,ActivityIndicator,Alert,Dimensions ,TextInput,TouchableOpacity,TouchableHighlight} from 'react-native';
...
this.props.navigation.navigate('Healthcare')
 ...
 getMoviesFromApiAsync = () => {
  this.showLoading()
      const url = GLOBAL.BASE_URL + 'industry_wise_jobs';
 ...
 buttonClickListener=()=>{
  ...
  _renderItem = ({item,index}) => {
    ...
    {GLOBAL.language == "en" && iSApply == "1" && (
                ...
                <Button
               containerStyle={{width:window.width - 20,marginLeft : -72,marginTop : 8,padding:12, height:45, overflow:'hidden', borderRadius:4, backgroundColor: '#261650'}}
...
<Image style={{marginLeft :10,marginTop :12,height :25,width :25,resizeMode:'contain'}}
               source={require('./back.png')} />
                 ...
                 {GLOBAL.language == "en" && (
               <Text style={{marginLeft : 15,marginTop:12,fontSize : 16,color :'white',fontFamily :'Poppins-Medium'}} >{GLOBAL.industryName} </Text>
             )}
             {GLOBAL.language != "en" && (
           <PowerTranslator style={{marginLeft : 15,marginTop:12,fontSize : 16,color :'white',fontFamily :'Poppins-Medium'}} text={GLOBAL.industryName} />
         )}
<TouchableOpacity style={{width:25, height:25,position:'absolute', right:10, top:12}} onPress={()=> this.props.navigation.navigate('SetFilter')}>
  <Image style={{width:25, height:25, resizeMode:'contain'}} source={require('./search.png')}/>
</TouchableOpacity>
</View>
