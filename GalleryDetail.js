...
<TouchableOpacity onPress = {()=>this.resPress(item)}>
    ...
    <Image style={{margin : 10,height :90,width :window.width/3 - 30,marginLeft: 5,resizeMode:'contain'}}
                 source={{uri: item.image}} />
                     {GLOBAL.language == "en" && (
<Text style={{margin:8,marginTop : - 8,alignSelf :'center',fontSize : 10,color :'black',fontFamily :'Poppins-Medium'}}>{item.caption} </Text>

)}

{GLOBAL.language != "en" && (
<PowerTranslator style={{margin:8,marginTop : - 8,alignSelf :'center',fontSize : 10,color :'black',fontFamily :'Poppins-Medium'}} text={item.caption} />
...
this.showLoading()
  ...
  const url = GLOBAL.BASE_URL + 'list_gallery_photo';
//     alert(url)
 fetch(url, {
method: 'POST',
headers: {
